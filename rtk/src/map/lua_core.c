...
int errors;
lua_State* lua_gstate;

// TODO: actually use this!
#define sl_ensureasync(state) \
	if(state == lua_gstate) { luaL_error(state, "this function must be called from a coroutine; " \
		"that is, the calling method must be marked with async()"); }

// TODO: do we even need this anymore?
#define lua_type_err_invalidattr(state, type, attrname) \
	luaL_error(state, "'%s' object has no attribute '%s'", type->name, attrname)
void lua_type_staticdestroy(); // TODO: this should unref lua_type_mtref?

int lua_blocklist_getattr(lua_State*, struct block_list*, char* attrname);
int lua_blocklist_setattr(lua_State*, struct block_list*, char* attrname);
void lua_blocklist_pushinst(lua_State*, struct block_list*, void*);

#define pcl_pushinst(state, sd) lua_type_pushinst(state, &lua_player_type, sd, 0)
lua_type_class lua_player_type;
void lua_player_staticinit();
#define regl_pushinst(state, sd) lua_type_pushinst(state, &regl_type, sd, 0)
lua_type_class regl_type;
void lua_registry_staticinit();

#define reglstring_pushinst(state, sd) lua_type_pushinst(state, &reglstring_type, sd, 0)
lua_type_class reglstring_type;
void lua_registry_string_staticinit();

#define npcintregl_pushinst(state, sd) lua_type_pushinst(state, &npcintregl_type, sd, 0)
lua_type_class npcintregl_type;
void npcintlua_registry_staticinit();
#define questregl_pushinst(state, sd) lua_type_pushinst(state, &questregl_type, sd, 0)
lua_type_class questregl_type;
void questlua_registry_staticinit();
#define npcregl_pushinst(state, nd) lua_type_pushinst(state, &npcregl_type, nd, 0)
lua_type_class npcregl_type;
void npclua_registry_staticinit();
#define mobregl_pushinst(state, mob) lua_type_pushinst(state, &mobregl_type, mob, 0)
lua_type_class mobregl_type;
void moblua_registry_staticinit();
#define fll_pushinst(state,fl) lua_type_pushinst(state, &fll_type, fl, 0)
lua_type_class fll_type;
void lua_floor_item_staticinit();
#define mapregl_pushinst(state,sd) lua_type_pushinst(state,&mapregl_type,sd,0)
lua_type_class mapregl_type;
void maplua_registry_staticinit();
#define gameregl_pushinst(state, sd) lua_type_pushinst(state, &gameregl_type,sd, 0)
lua_type_class gameregl_type;
void gamelua_registry_staticinit();

/*#define acctregl_pushinst(state, sd) lua_type_pushinst(state, &acctregl_type, sd, 0)
lua_type_class acctregl_type;
void acctlua_registry_staticinit();*/

#define mobl_pushinst(state, mob) lua_type_pushinst(state, &lua_mob_type, mob, 0)
lua_type_class lua_mob_type;
void lua_mob_staticinit();
#define npcl_pushinst(state, nd) lua_type_pushinst(state, &nlua_player_type, nd, 0)
lua_type_class nlua_player_type;
void nlua_player_staticinit();
//#define cnpcl_pushinst(state, cnd) lua_type_pushinst(state, &cnlua_player_type, cnd, 0)
//lua_type_class cnlua_player_type;
//void cnlua_player_staticinit();
#define iteml_pushinst(state, item) lua_type_pushinst(state, &iteml_type, item, 0)
lua_type_class iteml_type;
void lua_item_staticinit();
#define recipel_pushinst(state, recipe) lua_type_pushinst(state, &recipel_type, recipe, 0)
lua_type_class recipel_type;
void lua_recipe_staticinit();
#define biteml_pushinst(state, bitem) lua_type_pushinst(state, &biteml_type, bitem, 0)
lua_type_class biteml_type;
void blua_item_staticinit();

#define bankiteml_pushinst(state, bankitem) lua_type_pushinst(state, &bankiteml_type, bankitem, 0)
lua_type_class bankiteml_type;
void banklua_item_staticinit();

#define parcell_pushinst(state, parcel) lua_type_pushinst(state, &parcell_type, parcel, 0)
lua_type_class parcell_type;
void lua_parcel_staticinit();
#define SL_EXPOSE_ENUM(val) lua_pushnumber(lua_gstate, val); lua_setglobal(lua_gstate, #val)
...
int lua_blocklist_spawn(lua_State* state, struct block_list* bl) {
	...
	if (lua_isnumber(state, lua_memberarg(1))) {
		sl_checkargs(state, "nnnn");
		mob = lua_tonumber(state, lua_memberarg(1));
	}
	else {
		sl_checkargs(state, "snnn");
		mob = mobdb_id(lua_tostring(state, lua_memberarg(1)));
	}

	int x = lua_tonumber(state, lua_memberarg(2));
	int y = lua_tonumber(state, lua_memberarg(3));
	unsigned int amount = lua_tonumber(state, lua_memberarg(4));
	int m = lua_tonumber(state, lua_memberarg(5));
	unsigned int owner = lua_tonumber(state, lua_memberarg(6));
...
int lua_blocklist_getaliveobjects(lua_State* state, void* self, int area) {
	...
	/*#define NUMBER_FIELD(name) \
		lua_getfield(state, lua_memberself, #name); \
	...
	lua_newtable(state);
	int type = lua_tonumber(state, lua_memberarg(1));
...
int lua_blocklist_getobjects_cell(lua_State* state, void* self) {
	sl_checkargs(state, "nnnn");
	int m = lua_tonumber(state, lua_memberarg(1)),
		x = lua_tonumber(state, lua_memberarg(2)),
		y = lua_tonumber(state, lua_memberarg(3)),
		type = lua_tonumber(state, lua_memberarg(4));
	int def[1];
	def[0] = 0;
	lua_newtable(state);
...
int lua_blocklist_getobjects_map(lua_State* state, void* self) {
	...
	lua_newtable(state);
	int m = lua_tonumber(state, lua_memberarg(1));
	int type = lua_tonumber(state, lua_memberarg(2));
...
int lua_blocklist_getblock(lua_State* state, void* self) {
	sl_checkargs(state, "n");
	struct block_list* bl = map_id2bl(lua_tonumber(state, lua_memberarg(1)));
...
int lua_blocklist_sendanimxy(lua_State* state, void* self) {
	...
	struct block_list* bl = (struct block_list*)self;
	int anim = lua_tonumber(state, lua_memberarg(1));
	int x = lua_tonumber(state, lua_memberarg(2));
	int y = lua_tonumber(state, lua_memberarg(3));
	int times = lua_tonumber(state, lua_memberarg(4));
...
int lua_blocklist_sendanim(lua_State* state, void* self) {
	...
	struct block_list* bl = (struct block_list*)self;
	int anim = lua_tonumber(state, lua_memberarg(1));
	int times = lua_tonumber(state, lua_memberarg(2));
...
int lua_blocklist_selfanimationxy(lua_State* state, struct block_list* bl) {
	USER* sd = NULL;
	int anim = lua_tonumber(state, lua_memberarg(2));
	int x = lua_tonumber(state, lua_memberarg(3));
	int y = lua_tonumber(state, lua_memberarg(4));
	int times = lua_tonumber(state, lua_memberarg(5));

	if (lua_isnumber(state, lua_memberarg(1))) {
		sl_checkargs(state, "nnnn");
		sd = map_id2sd(lua_tonumber(state, lua_memberarg(1)));
	}
	else {
		sl_checkargs(state, "snnn");
		sd = map_name2sd(lua_tostring(state, lua_memberarg(1)));
	}
...
int lua_blocklist_selfanimation(lua_State* state, struct block_list* bl) {
	USER* sd = NULL;
	int anim = lua_tonumber(state, lua_memberarg(2));
	int times = lua_tonumber(state, lua_memberarg(3));
...
int lua_blocklist_sendaction(lua_State* state, void* self) {
	sl_checkargs(state, "nn");
	struct block_list* bl = (struct block_list*)self;
	int action = lua_tonumber(state, lua_memberarg(1));
	int time = lua_tonumber(state, lua_memberarg(2));
...
int lua_blocklist_throw(lua_State* state, void* self) {
	...
	char buf[255];
	x = lua_tonumber(state, lua_memberarg(1));
	y = lua_tonumber(state, lua_memberarg(2));
	icon = lua_tonumber(state, lua_memberarg(3));
	color = lua_tonumber(state, lua_memberarg(4));
	action = lua_tonumber(state, lua_memberarg(5));
	struct block_list* bl = (struct block_list*)self;
...
int getobjectsmap(lua_State* state) {
	...
	//lua_newtable(state);
	//printf("Map: %d\n",type);
	//map_foreachinarea(lua_blocklist_getobjects_helper, m, 1, 1, SAMEMAP, type, state);
	/*for(x=1;x<map[m].block_mob_count+;x++) {
		te=map[m].block_mob[x];
		lua_blocklist_pushinst(state,&te->bl,0);
...
void sl_checkargs(lua_State* state, char* fmt) {
	...
	for (narg = 1; fmt[narg - 1]; ++narg) {
		int expectedtype = LUA_TNIL, gottype = lua_type(state, lua_memberarg(narg));
		switch (fmt[narg - 1]) {
		...
		if (gottype != expectedtype) {
			...
			luaL_argerror(state, lua_memberarg(narg), message);
		}
	}
}

int lua_exec_redirectio(lua_State* state) {
	USER* sd = lua_touserdata(state, lua_upvalueindex(1));
	char* msg = lua_tostring(state, 1);
	int msglen = lua_objlen(state, 1);
	client_send_whisper(sd, "lua", msg);
	return 0;
}
int lua_luasize(USER* sd) {
	char buf[255];

	sprintf(buf, "LUA is using %u KB", lua_gc(lua_gstate, LUA_GCCOUNT, 0));
	client_send_minitext(sd, buf);
	return 0;
}

int lua_updatepeople(struct block_list* bl, va_list ap) {
	USER* sd = NULL;
	int x = 0;
	int y = 0;
	...
	client_sendmapdata(sd, sd->bl.m, x - 9, y - 8, 19, 17, 0);
	return 0;
}
...
int setobject(lua_State* state) {
	...
	if (map_isloaded(m)) {
		...
		map_foreachinarea(lua_updatepeople, m, x, y, AREA, BL_PC);
	}
...
int setmap(lua_State* state) {
	...
	fclose(fp);
	map_loadregistry(m);
	map_foreachinarea(lua_updatepeople, m, 0, 0, SAMEMAP, BL_PC);
	return 0;
}
...
void lua_exec(USER* sd, char* str) {
#define lua_exec_err() \
	char *msg = lua_tostring(lua_gstate, -1); \
	int msglen = lua_objlen(lua_gstate, -1); \
	client_send_msg(sd,0, msg)

	if (luaL_loadstring(lua_gstate, str) != 0) {
		lua_exec_err();
		lua_pop(lua_gstate, 1);
		return;
	}
	lua_newtable(lua_gstate); // the environment table
	lua_pushlightuserdata(lua_gstate, sd);
	lua_pushcclosure(lua_gstate, lua_exec_redirectio, 1);
	lua_setfield(lua_gstate, -2, "print"); // set the new print
	lua_newtable(lua_gstate); // the metatable for the environment
	lua_pushvalue(lua_gstate, LUA_GLOBALSINDEX);
	lua_setfield(lua_gstate, -2, "__index"); // set __index = _G
	lua_setmetatable(lua_gstate, -2);
	lua_setfenv(lua_gstate, -2);
	if (lua_pcall(lua_gstate, 0, 0, 0) != 0) {
		lua_exec_err();
...
int sl_guitext(lua_State* state, USER* sd) {
	...
	if (type == -1 && m == 0) { // This check is so Server Broadcast (by passing -1 into first argument and 0 for mapid) will take priority over a game.
		for (i = 0; i < 65535; i++) {
			if (map_isloaded(i)) {
				map_foreachinarea(client_gui_text, i, 0, 0, SAMEMAP, BL_PC, string, sd);
			}
		}
	}

	else { map_foreachinarea(client_gui_text, m, 0, 0, SAMEMAP, BL_PC, string, sd); }
...
void lua_binding_init() {
	int i, count = 0;
	lua_gstate = lua_open();
	luaL_openlibs(lua_gstate);

	//lua_gc(lua_gstate,LUA_GCSTOP,0); //TEMPORARILY IN..THIS MAKES YIELD WORK
	lua_type_staticinit();
	lua_player_staticinit();
	lua_registry_staticinit();
	lua_registry_string_staticinit();
	npcintlua_registry_staticinit();
	questlua_registry_staticinit();
	npclua_registry_staticinit();
	moblua_registry_staticinit();
	maplua_registry_staticinit();
	gamelua_registry_staticinit();
	//acctlua_registry_staticinit();
	lua_mob_staticinit();
	nlua_player_staticinit();
	lua_item_staticinit();
	lua_recipe_staticinit();
	blua_item_staticinit();
	banklua_item_staticinit();
	lua_parcel_staticinit();
	lua_floor_item_staticinit();
	...
	lua_register(lua_gstate, "_async", sl_async);
	lua_register(lua_gstate, "_asyncDone", sl_async_done);
	lua_register(lua_gstate, "getXPforLevel", sl_getXPforLevel);
	//lua_register(lua_gstate, "processKanDonations", sl_processKanDonations);
	lua_register(lua_gstate, "guitext", sl_guitext);
	lua_register(lua_gstate, "addToBoard", sl_addtoboard);
	lua_register(lua_gstate, "selectBulletinBoard", sl_selectBulletinBoard);
	lua_register(lua_gstate, "getPoems", sl_getPoems);
	lua_register(lua_gstate, "clearPoems", sl_clearPoems);
	lua_register(lua_gstate, "copyPoemToPoetry", sl_copyPoemToPoetry);
	lua_register(lua_gstate, "getMobAttributes", sl_getMobAttributes);
	lua_register(lua_gstate, "getAuctions", sl_getAuctions);
	lua_register(lua_gstate, "listAuction", sl_listAuction);
	lua_register(lua_gstate, "removeAuction", sl_removeAuction);
	lua_register(lua_gstate, "getOfflineID", getofflineid);
	lua_register(lua_gstate, "getMapRegistry", sl_getMapRegistry);
	lua_register(lua_gstate, "setMapRegistry", sl_setMapRegistry);
	lua_register(lua_gstate, "getSetItems", sl_getSetItems);
	lua_register(lua_gstate, "getMapPvP", sl_getMapPvP);
	lua_register(lua_gstate, "setMapPvP", sl_setMapPvP);
	lua_register(lua_gstate, "addMob", addmobspawn);

	// CLAN FUNCTIONS
	lua_register(lua_gstate, "getClanName", sl_getClanName);
	lua_register(lua_gstate, "setClanName", sl_setClanName);
	lua_register(lua_gstate, "getClanTribute", sl_getClanTribute);
	lua_register(lua_gstate, "setClanTribute", sl_setClanTribute);
	lua_register(lua_gstate, "addClanTribute", sl_addClanTribute);
	lua_register(lua_gstate, "getClanBankSlots", sl_getClanBankSlots);
	lua_register(lua_gstate, "setClanBankSlots", sl_setClanBankSlots);
	lua_register(lua_gstate, "getClanRoster", sl_getClanRoster);
	lua_register(lua_gstate, "removeClanMember", sl_removeClanMember);
	lua_register(lua_gstate, "addClanMember", sl_addClanMember);
	lua_register(lua_gstate, "updateClanMemberRank", sl_updateClanMemberRank);
	lua_register(lua_gstate, "updateClanMemberTitle", sl_updateClanMemberTitle);

	// SUBPATH FUNCTIONS
	lua_register(lua_gstate, "removePathMember", sl_removePathMember);
	lua_register(lua_gstate, "addPathMember", sl_addPathMember);

	// WISDOM STAR
	lua_register(lua_gstate, "setWisdomStarMultiplier", sl_setWisdomStarMultiplier);
	lua_register(lua_gstate, "getWisdomStarMultiplier", sl_getWisdomStarMultiplier);
	//lua_register(lua_gstate, "setKanDonationPoints", sl_setKanDonationPoints);
	//lua_register(lua_gstate, "getKanDonationPoints", sl_getKanDonationPoints);
	//lua_register(lua_gstate, "addKanDonationPoints", sl_addKanDonationPoints);

	// INSTANCE SYSTEM
	lua_register(lua_gstate, "getMapModifiers", sl_getMapModifiers);
	lua_register(lua_gstate, "addMapModifier", sl_addMapModifier);
	lua_register(lua_gstate, "removeMapModifier", sl_removeMapModifier);
	lua_register(lua_gstate, "removeMapModifierId", sl_removeMapModifierId);
	lua_register(lua_gstate, "getFreeMapModifierId", sl_getFreeMapModifierId);

	lua_register(lua_gstate, "setOfflinePlayerRegistry", sl_setOfflinePlayerRegistry);

	lua_register(lua_gstate, "getMapTitle", sl_getMapTitle);
	lua_register(lua_gstate, "setMapTitle", sl_setMapTitle);
	lua_register(lua_gstate, "getObjectsMap", getobjectsmap);
	lua_register(lua_gstate, "getObject", getobject);
	lua_register(lua_gstate, "setObject", setobject);
	lua_register(lua_gstate, "getTile", gettile);
	lua_register(lua_gstate, "setTile", settile);
	lua_register(lua_gstate, "setPass", setpass);
	lua_register(lua_gstate, "getPass", getpass);
	lua_register(lua_gstate, "setMap", setmap);
	lua_register(lua_gstate, "getMapAttribute", getMapAttribute);
	lua_register(lua_gstate, "setMapAttribute", setMapAttribute);
	lua_register(lua_gstate, "setPostColor", setpostcolor);
	lua_register(lua_gstate, "throw", throw);
	lua_register(lua_gstate, "msleep", msleep);
	lua_register(lua_gstate, "getWeather", getweather);
	lua_register(lua_gstate, "setWeather", setweather);
	lua_register(lua_gstate, "getWeatherM", getweatherm);
	lua_register(lua_gstate, "setWeatherM", setweatherm);
	lua_register(lua_gstate, "setLight", setlight);
	lua_register(lua_gstate, "getMapXMax", getMapXMax);
	lua_register(lua_gstate, "getMapYMax", getMapYMax);
	lua_register(lua_gstate, "getMapIsLoaded", getMapIsLoaded);
	lua_register(lua_gstate, "getMapUsers", getMapUsers);
	lua_register(lua_gstate, "getTick", getTick);
	lua_register(lua_gstate, "curServer", curserver);
	lua_register(lua_gstate, "curYear", curyear);
	lua_register(lua_gstate, "curSeason", curseason);
	lua_register(lua_gstate, "curDay", curday);
	lua_register(lua_gstate, "curTime", curtime);
	lua_register(lua_gstate, "realDay", realday);
	lua_register(lua_gstate, "realHour", realhour);
	lua_register(lua_gstate, "realMinute", realminute);
	lua_register(lua_gstate, "realSecond", realsecond);
	lua_register(lua_gstate, "broadcast", broadcast);
	lua_register(lua_gstate, "gmbroadcast", gmbroadcast);
	lua_register(lua_gstate, "checkOnline", checkonline);
	lua_register(lua_gstate, "luaReload", luareload);
	lua_register(lua_gstate, "timeMS", luatimems);
	lua_register(lua_gstate, "getWarp", getwarp);
	lua_register(lua_gstate, "getWarps", getwarps);
	lua_register(lua_gstate, "setWarps", setwarps);
	lua_register(lua_gstate, "saveMap", savemap);
	lua_register(lua_gstate, "getSpellLevel", getSpellLevel);
	lua_register(lua_gstate, "sendMeta", sendMeta);
	lua_atpanic(lua_gstate, func_panic);
	// make sure to load the sys.lua file before anything else!
	if (luaL_dofile(lua_gstate, "../rtklua/Developers/sys.lua")) {
...
int sl_loaddir(char* dirpath, lua_State* state) {
	...
	while (0 != (dir_entry = readdir(dir))) {
		...
		if (S_ISDIR(dir_entry_stat.st_mode)) {
			...
			if (luaL_dofile(lua_gstate, path)) {
				errors++;

				if (state != NULL) {
					lua_pushstring(state, lua_tostring(lua_gstate, -1));
...
int lua_reload(lua_State* state) {
	int errors = 0;
...
void lua_fixmem() {
	lua_gc(lua_gstate, LUA_GCCOLLECT, 0);
	return 0;
}
int sl_async_done(lua_State* state) {
	// free the coroutine
	USER* sd = lua_type_topointer(state, 1);
	lua_async_freeco(sd);
}
void lua_async_freeco(USER* sd) {
	if (!sd->coref) return 0;
	luaL_unref(lua_gstate, LUA_REGISTRYINDEX, sd->coref);
	sd->coref = 0;
}
void sl_async_resume(USER* sd, int nargs) {
	...
	lua_State* costate;
	lua_pushref(lua_gstate, coref);
	if (lua_isthread(lua_gstate, -1)) {
		costate = lua_tothread(lua_gstate, -1);
		lua_pop(lua_gstate, 1); // pop the thread
		//printf("%d\n",lua_status(costate));
		lua_xmove(lua_gstate, costate, nargs);
...
int sl_async(lua_State* state) {
	...
	if (sd->coref) { //client_send_minitext(sd,"You are busy.");
		return 0;
	};
...
int sl_doscript_strings(char* root, char* method, int nargs, ...) {
	...
	va_start(argp, nargs);
	for (i = 0; i < nargs; i++) {
		lua_pushstring(lua_gstate, va_arg(argp, char*));
	}
	va_end(argp);
...
void lua_resume_menu(unsigned int selection, USER* sd) {
	/*if(sd->coroutine) {
	...
	lua_pushnumber(lua_gstate, selection);
...
void lua_resume_menuseq(unsigned int selection, int choice, USER* sd) {
	...
	if (selection == 2) { //Selected(Next)
		lua_pushnumber(lua_gstate, choice);
		sl_async_resume(sd, 1);
	}
}

void lua_resume_dialog(unsigned int choice, USER* sd) {
	/*if(sd->coroutine) {
		if(lua_status(sd->coroutine)==LUA_YIELD) {
	...
	if (choice == 0) { lua_pushstring(lua_gstate, "previous"); }
	//else if(choice == 1) { lua_pushstring(lua_gstate, "quit"); }
	else if (choice == 1) { lua_async_freeco(sd); return; }
	else if (choice == 2) { lua_pushstring(lua_gstate, "next"); }
	else { lua_pushstring(lua_gstate, "quit"); }
	sl_async_resume(sd, 1);
}
void lua_resume_inputseq(unsigned int choice, char* input, USER* sd) {
	...
	else if (choice == 2) { lua_pushstring(lua_gstate, "next"); lua_pushstring(lua_gstate, input); }
	else { lua_pushstring(lua_gstate, "quit"); }
	sl_async_resume(sd, 1);
}

/*void lua_resume_buy(unsigned int choice, USER *sd) {
	lua_pushnumber(lua_gstate, choice);
	sl_async_resume(sd, 1);
}*/

void lua_resume_buy(char* name, USER* sd) {
	lua_pushstring(lua_gstate, name);
	sl_async_resume(sd, 1);
}

void lua_resume_sell(unsigned int choice, USER* sd) {
	/*if(sd->coroutine) {
		if(lua_status(sd->coroutine)==LUA_YIELD) {
...
void lua_resume_input(char* output, char* output2, USER* sd) {
	/*if(sd->coroutine) {
		if(lua_status(sd->coroutine)==LUA_YIELD) {
	...
	lua_pushstring(lua_gstate, output2);
	sl_async_resume(sd, 1);
}
...
void sl_resume(char* output, USER* sd) {
	...
	lua_pushstring(lua_gstate, output);
	sl_async_resume(sd, 1);
}
...
void lua_item_staticinit() {
	iteml_type = lua_type_new("Item", iteml_ctor);
...
void blua_item_staticinit() {
	biteml_type = lua_type_new("BoundItem", 0);
...
void banklua_item_staticinit() {
	bankiteml_type = lua_type_new("BankItem", 0);
...
void lua_recipe_staticinit() {
	recipel_type = lua_type_new("Recipe", recipel_ctor);
...
void lua_parcel_staticinit() {
	parcell_type = lua_type_new("Parcel", 0);
...
void lua_registry_staticinit() {
	regl_type = lua_type_new("Registry", 0);
...
void lua_registry_string_staticinit() {
	reglstring_type = lua_type_new("RegistryString", 0);
...
void npcintlua_registry_staticinit() {
	npcintregl_type = lua_type_new("NpcInt", 0);
...
void questlua_registry_staticinit() {
	questregl_type = lua_type_new("Quest", 0);
...
/* lua_type_* functions moved to sl_types.c */
...
int lua_blocklist_playsound(lua_State* state, struct block_list* bl) {
	sl_checkargs(state, "n");
	int sound = lua_tonumber(state, lua_memberarg(1));
...
int lua_blocklist_talk(lua_State* state, struct block_list* bl) {
	sl_checkargs(state, "ns");
	int type = lua_tonumber(state, lua_memberarg(1));
	char* msg = lua_tostring(state, lua_memberarg(2));
...
int lua_blocklist_talkcolor(lua_State* state, struct block_list* bl) {
	sl_checkargs(state, "ns");
	int color = lua_tonumber(state, lua_memberarg(1));
	char* msg = lua_tostring(state, lua_memberarg(2));
	USER* tsd = NULL;

	if (lua_isnumber(state, lua_memberarg(3))) {
		sl_checkargs(state, "nsn");
		tsd = map_id2sd(lua_tonumber(state, lua_memberarg(3)));
	}
	else {
		sl_checkargs(state, "nss");
		tsd = map_name2sd(lua_tostring(state, lua_memberarg(3)));
	}

	if (lua_tonumber(state, lua_memberarg(3)) == 0) {
...
int lua_blocklist_addnpc(lua_State* state, struct block_list* bl) {
	...
	timer = lua_tonumber(state, lua_memberarg(6));
	duration = lua_tonumber(state, lua_memberarg(7));
	owner = lua_tonumber(state, lua_memberarg(8));
	movetime = lua_tonumber(state, lua_memberarg(9));
	CALLOC(nd, struct npc_data, 1);

	strcpy(nd->name, lua_tostring(state, lua_memberarg(1)));
	if (lua_isstring(state, lua_memberarg(9))) {
		strcpy(nd->npc_name, lua_tostring(state, lua_memberarg(9)));
	}
	else {
		strcpy(nd->npc_name, "nothing");
	}
	nd->bl.type = BL_NPC;
	nd->bl.subtype = lua_tonumber(state, lua_memberarg(5));
	nd->bl.m = lua_tonumber(state, lua_memberarg(2));
	nd->bl.x = lua_tonumber(state, lua_memberarg(3));
	nd->bl.y = lua_tonumber(state, lua_memberarg(4));
	nd->bl.graphic_id = 0;
	...
	lua_do_script_blargs(nd->name, "on_spawn", 1, &nd->bl);
	return 0;
}
int lua_blocklist_dropitem(lua_State* state, struct block_list* bl) {
	...
	if (lua_isnumber(state, lua_memberarg(1))) {
		sl_checkargs(state, "nn");
		id = lua_tonumber(state, lua_memberarg(1));
	}
	else {
		sl_checkargs(state, "sn");
		id = itemdb_id(lua_tostring(state, lua_memberarg(1)));
	}

	amount = lua_tonumber(state, lua_memberarg(2));
	dura = lua_tonumber(state, lua_memberarg(3));
	protected = lua_tonumber(state, lua_memberarg(4));
...
int lua_blocklist_dropitemxy(lua_State* state, struct block_list* bl) {
	...
	map = lua_tonumber(state, lua_memberarg(5));
	x = lua_tonumber(state, lua_memberarg(6));
	y = lua_tonumber(state, lua_memberarg(7));
...
int lua_blocklist_objectcanmove(lua_State* state, struct block_list* bl) {
	sl_checkargs(state, "nnn");
	int x = lua_tonumber(state, lua_memberarg(1));
	int y = lua_tonumber(state, lua_memberarg(2));
	int side = lua_tonumber(state, lua_memberarg(3));
...
int lua_blocklist_repeatanimation(lua_State* state, struct block_list* bl) {
	...
	//MOB* mob=NULL;
	//USER* t=NULL;
	//int id=magicdb_id(lua_tostring(state,lua_memberarg(1)));
	int anim = lua_tonumber(state, lua_memberarg(1));
	int duration = lua_tonumber(state, lua_memberarg(2));
...
int lua_blocklist_sendparcel(lua_State* state, struct block_list* bl) {
	...
	int receiver = lua_tonumber(state, lua_memberarg(1));
	int sender = lua_tonumber(state, lua_memberarg(2));
	unsigned int item = lua_tonumber(state, lua_memberarg(3));
	unsigned int amount = lua_tonumber(state, lua_memberarg(4));
	int owner = lua_tonumber(state, lua_memberarg(5));
	char* engrave = lua_tostring(state, lua_memberarg(6));
	char npcflag = lua_tonumber(state, lua_memberarg(7));
	customLook = lua_tonumber(state, lua_memberarg(8));
	customLookColor = lua_tonumber(state, lua_memberarg(9));
	customIcon = lua_tonumber(state, lua_memberarg(10));
	customIconColor = lua_tonumber(state, lua_memberarg(11));
	protected = lua_tonumber(state, lua_memberarg(12));
	dura = lua_tonumber(state, lua_memberarg(13));
...
void lua_blocklist_extendproto(lua_type_class* class) {
	...
	//lua_type_extendproto(class, "checkOnline", lua_blocklist_checkonline);
...
void lua_blocklist_pushinst(lua_State* state, struct block_list* bl, void* param) {
	...
	if (bl->type == BL_PC) {
		...
		lua_type_pushinst(state, &lua_player_type, sd, param);
	}
	else if (bl->type == BL_MOB) {
		MOB* mob = (MOB*)bl;
		lua_type_pushinst(state, &lua_mob_type, mob, param);
	}
	else if (bl->type == BL_NPC) {
		struct npc_data* nd = (struct npc_data*)bl;
		lua_type_pushinst(state, &nlua_player_type, nd, param);
	}
...
void lua_floor_item_staticinit() {
	fll_type = lua_type_new("FloorItem", fll_ctor);
	fll_type.getattr = fll_getattr;
	fll_type.setattr = fll_setattr;
	fll_type.init = fll_init;

	lua_type_extendproto(&fll_type, "getTrapSpotters", fll_getTrapSpotters);
	lua_type_extendproto(&fll_type, "addTrapSpotters", fll_addTrapSpotters);

	/*lua_type_extendproto(&lua_mob_type, "attack", mobl_attack);
	lua_type_extendproto(&lua_mob_type, "addHealth", mobl_addhealth);
	lua_type_extendproto(&lua_mob_type, "move",mobl_move);
	lua_type_extendproto(&lua_mob_type,"setDuration",mobl_setduration);
	lua_type_extendproto(&lua_mob_type,"moveIntent",mobl_moveintent);
	lua_type_extendproto(&lua_mob_type, "hasDuration",mobl_hasduration);
	lua_type_extendproto(&lua_mob_type,"removeHealth",mobl_removehealth);
	lua_type_extendproto(&lua_mob_type,"flushDuration",mobl_flushduration);
	lua_type_extendproto(&lua_mob_type,"durationAmount",mobl_durationamount);
	*/
	lua_blocklist_extendproto(&fll_type);
}
...
int fll_getattr(lua_State* state, FLOORITEM* fl, char* attrname) {
	if (lua_blocklist_getattr(state, &fl->bl, attrname)) return 1;
...
int fll_setattr(lua_State* state, FLOORITEM* fl, char* attrname) {
	if (lua_blocklist_setattr(state, &fl->bl, attrname)) return 1;
	if (!strcmp(attrname, "amount")) fl->data.amount = lua_tonumber(state, -1);
		...
		while (lua_next(state, lua_memberarg(2)) != 0) {
...
int fll_addTrapSpotters(lua_State* state, FLOORITEM* fl) {
	unsigned int playerid = lua_tonumber(state, lua_memberarg(1));
...
void nlua_player_staticinit() {
	nlua_player_type = lua_type_new("NPC", npcl_ctor);
	nlua_player_type.init = npcl_init;
	nlua_player_type.getattr = npcl_getattr;
	nlua_player_type.setattr = npcl_setattr;
	lua_type_extendproto(&nlua_player_type, "move", npcl_move);
	lua_type_extendproto(&nlua_player_type, "warp", npcl_warp);
	lua_type_extendproto(&nlua_player_type, "getEquippedItem", npcl_getequippeditem);
	lua_blocklist_extendproto(&nlua_player_type);
}

void npclua_registry_staticinit() {
	npcregl_type = lua_type_new("Registry", 0);
...
int npcl_getattr(lua_State* state, NPC* nd, char* attrname) {
	if (lua_blocklist_getattr(state, &nd->bl, attrname)) return 1;
...
int npcl_setattr(lua_State* state, NPC* nd, char* attrname) {
	if (lua_blocklist_setattr(state, &nd->bl, attrname)) return 1;
...
int npcl_warp(lua_State* state, NPC* nd) {
	...
	m = lua_tonumber(state, lua_memberarg(1));
	x = lua_tonumber(state, lua_memberarg(2));
	y = lua_tonumber(state, lua_memberarg(3));
...
int npcl_getequippeditem(lua_State* state, NPC* nd) {
	sl_checkargs(state, "n");
	int num = lua_tonumber(state, lua_memberarg(1));
	lua_newtable(state);
...
void lua_mob_staticinit() {
	lua_mob_type = lua_type_new("Mob", mobl_ctor);
	lua_mob_type.getattr = mobl_getattr;
	lua_mob_type.setattr = mobl_setattr;
	mobregl_type.getattr = mobregl_getattr;
	mobregl_type.setattr = mobregl_setattr;
	lua_mob_type.init = mobl_init;
	lua_type_extendproto(&lua_mob_type, "attack", mobl_attack);
	lua_type_extendproto(&lua_mob_type, "addHealth", mobl_addhealth);
	lua_type_extendproto(&lua_mob_type, "move", mobl_move);
	lua_type_extendproto(&lua_mob_type, "moveIgnoreObject", mobl_move_ignore_object);
	lua_type_extendproto(&lua_mob_type, "setDuration", mobl_setduration);
	lua_type_extendproto(&lua_mob_type, "moveIntent", mobl_moveintent);
	lua_type_extendproto(&lua_mob_type, "hasDuration", mobl_hasduration);
	lua_type_extendproto(&lua_mob_type, "hasDurationID", mobl_hasdurationid); //hasduration with id check
	lua_type_extendproto(&lua_mob_type, "getDuration", mobl_getduration);
	lua_type_extendproto(&lua_mob_type, "getDurationID", mobl_getdurationid);
	lua_type_extendproto(&lua_mob_type, "removeHealth", mobl_removehealth);
	lua_type_extendproto(&lua_mob_type, "flushDuration", mobl_flushduration);
	lua_type_extendproto(&lua_mob_type, "flushDurationNoUncast", mobl_flushdurationnouncast);
	lua_type_extendproto(&lua_mob_type, "durationAmount", mobl_durationamount);
	lua_type_extendproto(&lua_mob_type, "checkThreat", mobl_checkthreat);
	lua_type_extendproto(&lua_mob_type, "sendHealth", mobl_sendhealth);
	lua_type_extendproto(&lua_mob_type, "warp", mobl_warp);
	lua_type_extendproto(&lua_mob_type, "moveGhost", mobl_moveghost);
	lua_type_extendproto(&lua_mob_type, "callBase", mobl_callbase);
	lua_type_extendproto(&lua_mob_type, "checkMove", mobl_checkmove);
	lua_type_extendproto(&lua_mob_type, "setIndDmg", mobl_setinddmg);
	lua_type_extendproto(&lua_mob_type, "setGrpDmg", mobl_setgrpdmg);
	lua_type_extendproto(&lua_mob_type, "getIndDmg", mobl_getinddmg);
	lua_type_extendproto(&lua_mob_type, "getGrpDmg", mobl_getgrpdmg);
	lua_type_extendproto(&lua_mob_type, "getEquippedItem", mobl_getequippeditem);
	lua_type_extendproto(&lua_mob_type, "calcStat", mobl_calcstat);
	lua_type_extendproto(&lua_mob_type, "sendStatus", mobl_sendstatus);
	lua_type_extendproto(&lua_mob_type, "sendMinitext", mobl_sendminitext);

	lua_blocklist_extendproto(&lua_mob_type);
}
void moblua_registry_staticinit() {
	mobregl_type = lua_type_new("Registry", 0);
	mobregl_type.getattr = mobregl_getattr;
	mobregl_type.setattr = mobregl_setattr;
}
int mobl_durationamount(lua_State* state, MOB* mob) {
	sl_checkargs(state, "n");
	int x, id;
	id = magicdb_id(lua_tostring(state, lua_memberarg(1)));
...
int mobl_removehealth(lua_State* state, MOB* mob) {
	sl_checkargs(state, "n");
	int damage = lua_tonumber(state, lua_memberarg(1));
	int caster = lua_tonumber(state, lua_memberarg(2));
...
int mobl_setduration(lua_State* state, MOB* mob) {
	...
	alreadycast = 0;
	id = magicdb_id(lua_tostring(state, lua_memberarg(1)));
	time = lua_tonumber(state, lua_memberarg(2));
	if (time < 1000 && time > 0) time = 1000;
	caster_id = lua_tonumber(state, lua_memberarg(3)); //set duration for specific caster, 0 for universal
	recast = lua_tonumber(state, lua_memberarg(4)); //should the spell recast?
	...
	for (x = 0; x < MAX_MAGIC_TIMERS; x++) {
		...
		if (mid == id && time <= 0 && mob->da[x].caster_id == caster_id && alreadycast == 1) {
			...
			if (bl != NULL) {
				lua_do_script_blargs(magicdb_yname(mid), "uncast", 2, &mob->bl, bl);
			}
			else {
				lua_do_script_blargs(magicdb_yname(mid), "uncast", 1, &mob->bl);
			}
...
int mobl_flushduration(lua_State* state, MOB* mob) {
	...
	int x;
	int id;
	int dis = lua_tonumber(state, lua_memberarg(1));
	int minid = lua_tonumber(state, lua_memberarg(2));
	int maxid = lua_tonumber(state, lua_memberarg(3));
	char flush = 0;
	...
	for (x = 0; x < MAX_MAGIC_TIMERS; x++) {
		...
		if (flush == 1) {
			...
			if (bl != NULL) {
				lua_do_script_blargs(magicdb_yname(id), "uncast", 2, &mob->bl, bl);
			}
			else {
				lua_do_script_blargs(magicdb_yname(id), "uncast", 1, &mob->bl);
			}
		}
	}
...
int mobl_hasdurationid(lua_State* state, MOB* mob) {
	...
	id = magicdb_id(lua_tostring(state, lua_memberarg(1)));
	caster_id = lua_tonumber(state, lua_memberarg(2)); //check duration from specific caster
...
int mobl_getdurationid(lua_State* state, MOB* mob) {
	...
	caster_id = 0;
	id = magicdb_id(lua_tostring(state, lua_memberarg(1)));
	caster_id = lua_tonumber(state, lua_memberarg(2));
...
int mobl_sendhealth(lua_State* state, MOB* mob) {
	sl_checkargs(state, "nn");
	int damage;
	float dmg = lua_tonumber(state, lua_memberarg(1));
	int critical = lua_tonumber(state, lua_memberarg(2));
...
int mobl_init(lua_State* state, MOB* mob, int dataref, void* param) {
	...
	if (mob->target && param == 0) {
		...
		if(bl) {
			lua_blocklist_pushinst(state, bl, 1);
...
int mobl_attack(lua_State* state, MOB* mob) {
	sl_checkargs(state, "n");
	int id = lua_tonumber(state, lua_memberarg(1));
	mob_attack(mob, id);
	return 0;
}
	...
	if (bl != NULL) {
		if (damage > 0) {
			if (mob->data->subtype == 0) {
				lua_do_script_blargs("mob_ai_basic", "on_healed", 2, &mob->bl, bl);
			}
			else if (mob->data->subtype == 1) {
				lua_do_script_blargs("mob_ai_normal", "on_healed", 2, &mob->bl, bl);
			}
			else if (mob->data->subtype == 2) {
				lua_do_script_blargs("mob_ai_hard", "on_healed", 2, &mob->bl, bl);
			}
			else if (mob->data->subtype == 3) {
				lua_do_script_blargs("mob_ai_boss", "on_healed", 2, &mob->bl, bl);
			}
			else if (mob->data->subtype == 4) {
				lua_do_script_blargs(mob->data->yname, "on_healed", 2, &mob->bl, bl);
			}
			else if (mob->data->subtype == 5) {
				lua_do_script_blargs("mob_ai_ghost", "on_healed", 2, &mob->bl, bl);
			}
		}
	}
	else {
		if (damage > 0) {
			if (mob->data->subtype == 0) {
				lua_do_script_blargs("mob_ai_basic", "on_healed", 1, &mob->bl);
			}
			else if (mob->data->subtype == 1) {
				lua_do_script_blargs("mob_ai_normal", "on_healed", 1, &mob->bl);
			}
			else if (mob->data->subtype == 2) {
				lua_do_script_blargs("mob_ai_hard", "on_healed", 1, &mob->bl);
			}
			else if (mob->data->subtype == 3) {
				lua_do_script_blargs("mob_ai_boss", "on_healed", 1, &mob->bl);
			}
			else if (mob->data->subtype == 4) {
				lua_do_script_blargs(mob->data->yname, "on_healed", 1, &mob->bl);
			}
			else if (mob->data->subtype == 5) {
				lua_do_script_blargs("mob_ai_ghost", "on_healed", 1, &mob->bl);
			}
		}
	}
...
int mobl_callbase(lua_State* state, MOB* mob) {
	sl_checkargs(state, "s");
	char* script = lua_tostring(state, lua_memberarg(1));
	struct block_list* bl = map_id2bl(mob->attacker);

	if (bl != NULL) {
		lua_do_script_blargs(mob->data->yname, script, 2, &mob->bl, bl);
		lua_pushboolean(state, 1);
		return 1;
	}
	else {
		lua_do_script_blargs(mob->data->yname, script, 2, &mob->bl, &mob->bl);
		lua_pushboolean(state, 1);
		return 1;
	}
...
int mobl_setgrpdmg(lua_State* state, MOB* mob) {
	int x;
	USER* sd = map_id2sd(lua_tonumber(state, lua_memberarg(1)));
	float dmg = lua_tonumber(state, lua_memberarg(2));
...
void lua_player_staticinit() {
	lua_player_type = lua_type_new("Player", pcl_ctor);
	lua_player_type.getattr = pcl_getattr;
	lua_player_type.setattr = pcl_setattr;
	lua_player_type.init = pcl_init;
	lua_type_extendproto(&lua_player_type, "freeAsync", pcl_freeasync);
	lua_type_extendproto(&lua_player_type, "showHealth", pcl_showhealth);
	lua_type_extendproto(&lua_player_type, "addHealth", pcl_addhealth);
	lua_type_extendproto(&lua_player_type, "removeHealth", pcl_removehealth);
	lua_type_extendproto(&lua_player_type, "sendHealth", pcl_sendhealth);
	lua_type_extendproto(&lua_player_type, "die", pcl_die);
	lua_type_extendproto(&lua_player_type, "resurrect", pcl_resurrect);
	lua_type_extendproto(&lua_player_type, "setTimeValues", pcl_settimevalues);
	lua_type_extendproto(&lua_player_type, "getTimeValues", pcl_gettimevalues);
	lua_type_extendproto(&lua_player_type, "setPK", pcl_setpk);
	lua_type_extendproto(&lua_player_type, "getPK", pcl_getpk);
	lua_type_extendproto(&lua_player_type, "status", pcl_status);
	lua_type_extendproto(&lua_player_type, "sendStatus", pcl_sendstatus);
	lua_type_extendproto(&lua_player_type, "setHeroShow", pcl_setHeroShow);
	lua_type_extendproto(&lua_player_type, "setAccountBan", pcl_setAccountBan);
	lua_type_extendproto(&lua_player_type, "getCaptchaKey", pcl_getCaptchaKey);
	lua_type_extendproto(&lua_player_type, "setCaptchaKey", pcl_setCaptchaKey);
	lua_type_extendproto(&lua_player_type, "sendMiniMap", pcl_sendminimap);
	lua_type_extendproto(&lua_player_type, "addKan", pcl_addKan);
	lua_type_extendproto(&lua_player_type, "removeKan", pcl_removeKan);
	lua_type_extendproto(&lua_player_type, "setKan", pcl_setKan);
	lua_type_extendproto(&lua_player_type, "checkKan", pcl_checkKan);
	lua_type_extendproto(&lua_player_type, "claimKan", pcl_claimKan);
	lua_type_extendproto(&lua_player_type, "kanBalance", pcl_kanBalance);
	lua_type_extendproto(&lua_player_type, "updatePath", pcl_updatePath);

	lua_type_extendproto(&lua_player_type, "updateCountry", pcl_updateCountry);
	lua_type_extendproto(&lua_player_type, "updateMail", pcl_updateMail);
	//lua_type_extendproto(&lua_player_type, "addActivationKey",pcl_addActivationKey);
	//lua_type_extendproto(&lua_player_type, "checkActivationKey",pcl_checkActivationKey);
	lua_type_extendproto(&lua_player_type, "setMiniMapToggle", pcl_setMiniMapToggle);
	lua_type_extendproto(&lua_player_type, "calcStat", pcl_calcstat);
	lua_type_extendproto(&lua_player_type, "lookAt", pcl_lookat);
	//lua_type_extendproto(&lua_player_type, "giveXP",pcl_givexp);
	lua_type_extendproto(&lua_player_type, "sendURL", pcl_sendurl);
	lua_type_extendproto(&lua_player_type, "sendMinitext", pcl_sendminitext);
	lua_type_extendproto(&lua_player_type, "speak", pcl_speak);
	lua_type_extendproto(&lua_player_type, "talkSelf", pcl_talkself);
	lua_type_extendproto(&lua_player_type, "guitext", pcl_guitext);
	lua_type_extendproto(&lua_player_type, "sendMail", pcl_sendmail);
	lua_type_extendproto(&lua_player_type, "showBoard", pcl_showboard);
	lua_type_extendproto(&lua_player_type, "showPost", pcl_showpost);
	lua_type_extendproto(&lua_player_type, "refresh", pcl_refresh);
	lua_type_extendproto(&lua_player_type, "refreshInventory", pcl_refreshInventory);
	lua_type_extendproto(&lua_player_type, "popUp", pcl_popup);
	lua_type_extendproto(&lua_player_type, "paperpopup", pcl_paperpopup);
	lua_type_extendproto(&lua_player_type, "paperpopupwrite", pcl_paperpopupwrite);
	lua_type_extendproto(&lua_player_type, "input", pcl_input);
	lua_type_extendproto(&lua_player_type, "dialog", pcl_dialog);
	lua_type_extendproto(&lua_player_type, "menu", pcl_menu);
	lua_type_extendproto(&lua_player_type, "menuSeq", pcl_menuseq);
	lua_type_extendproto(&lua_player_type, "inputSeq", pcl_inputseq);
	lua_type_extendproto(&lua_player_type, "sendBoardQuestions", pcl_sendboardquestions);
	lua_type_extendproto(&lua_player_type, "buy", pcl_buy);
	lua_type_extendproto(&lua_player_type, "sell", pcl_sell);
	lua_type_extendproto(&lua_player_type, "logBuySell", pcl_logbuysell);
	lua_type_extendproto(&lua_player_type, "setDuration", pcl_setduration);
	lua_type_extendproto(&lua_player_type, "hasDuration", pcl_hasduration);
	lua_type_extendproto(&lua_player_type, "hasDurationID", pcl_hasdurationid);
	lua_type_extendproto(&lua_player_type, "getDuration", pcl_getduration);
	lua_type_extendproto(&lua_player_type, "getDurationID", pcl_getdurationid);
	lua_type_extendproto(&lua_player_type, "getAllDurations", pcl_getalldurations);
	lua_type_extendproto(&lua_player_type, "forceSave", pcl_forcesave);
	lua_type_extendproto(&lua_player_type, "refreshDurations", pcl_refreshdurations);
	lua_type_extendproto(&lua_player_type, "flushDuration", pcl_flushduration);
	//lua_type_extendproto(&lua_player_type, "flushDurationRange",pcl_flushdurationrange);
	lua_type_extendproto(&lua_player_type, "flushDurationNoUncast", pcl_flushdurationnouncast);
	lua_type_extendproto(&lua_player_type, "getCasterID", pcl_getcasterid);
	lua_type_extendproto(&lua_player_type, "setAether", pcl_setaether);
	lua_type_extendproto(&lua_player_type, "hasAether", pcl_hasaether); //check for aether
	lua_type_extendproto(&lua_player_type, "getAether", pcl_getaether);
	lua_type_extendproto(&lua_player_type, "getAllAethers", pcl_getallaethers);
	lua_type_extendproto(&lua_player_type, "flushAether", pcl_flushaether);
	lua_type_extendproto(&lua_player_type, "hasSpace", pcl_hasspace);
	lua_type_extendproto(&lua_player_type, "addGift", pcl_addGift);
	lua_type_extendproto(&lua_player_type, "retrieveGift", pcl_retrieveGift);
	lua_type_extendproto(&lua_player_type, "addItem", pcl_additem);
	lua_type_extendproto(&lua_player_type, "getInventoryItem", pcl_getinventoryitem);
	lua_type_extendproto(&lua_player_type, "getExchangeItem", pcl_getexchangeitem);
	lua_type_extendproto(&lua_player_type, "getBODItem", pcl_getboditem);
	lua_type_extendproto(&lua_player_type, "hasItem", pcl_hasitem);
	lua_type_extendproto(&lua_player_type, "hasItemDura", pcl_hasitemdura); //has item with full dura
	lua_type_extendproto(&lua_player_type, "removeItem", pcl_removeinventoryitem);
	lua_type_extendproto(&lua_player_type, "removeItemSlot", pcl_removeitemslot);
	lua_type_extendproto(&lua_player_type, "removeItemDura", pcl_removeitemdura); //remove items with full dura
	lua_type_extendproto(&lua_player_type, "getEquippedItem", pcl_getequippeditem);
	lua_type_extendproto(&lua_player_type, "hasEquipped", pcl_hasequipped);
	lua_type_extendproto(&lua_player_type, "pickUp", pcl_pickup);
	lua_type_extendproto(&lua_player_type, "equip", pcl_equip);
	lua_type_extendproto(&lua_player_type, "forceEquip", pcl_forceequip);
	lua_type_extendproto(&lua_player_type, "takeOff", pcl_takeoff);
	lua_type_extendproto(&lua_player_type, "stripEquip", pcl_stripequip);
	lua_type_extendproto(&lua_player_type, "throwItem", pcl_throwitem);
	lua_type_extendproto(&lua_player_type, "updateInv", pcl_updateinv);
	lua_type_extendproto(&lua_player_type, "addLegend", pcl_addlegend);
	lua_type_extendproto(&lua_player_type, "hasLegend", pcl_haslegend);
	lua_type_extendproto(&lua_player_type, "removeLegendbyName", pcl_removelegendbyname);
	lua_type_extendproto(&lua_player_type, "removeLegendbyColor", pcl_removelegendbycolor);
	lua_type_extendproto(&lua_player_type, "warp", pcl_warp);
	lua_type_extendproto(&lua_player_type, "forceDrop", pcl_forcedrop);
	lua_type_extendproto(&lua_player_type, "move", pcl_move);
	lua_type_extendproto(&lua_player_type, "respawn", pcl_respawn);
	lua_type_extendproto(&lua_player_type, "addClan", pcl_addclan);
	lua_type_extendproto(&lua_player_type, "killCount", pcl_killcount);
	lua_type_extendproto(&lua_player_type, "setKillCount", pcl_setkillcount);
	lua_type_extendproto(&lua_player_type, "flushAllKills", pcl_flushallkills);
	lua_type_extendproto(&lua_player_type, "flushKills", pcl_flushkills);
	lua_type_extendproto(&lua_player_type, "getUnknownSpells", pcl_getunknownspells);
	lua_type_extendproto(&lua_player_type, "getAllSpells", pcl_getallspells);
	lua_type_extendproto(&lua_player_type, "getAllClassSpells", pcl_getallclassspells);
	lua_type_extendproto(&lua_player_type, "addSpell", pcl_addspell);
	lua_type_extendproto(&lua_player_type, "removeSpell", pcl_removespell);
	lua_type_extendproto(&lua_player_type, "hasSpell", pcl_hasspell);
	lua_type_extendproto(&lua_player_type, "getSpells", pcl_getspells); //for remove spell
	lua_type_extendproto(&lua_player_type, "getSpellName", pcl_getspellname); //for remove spell
	lua_type_extendproto(&lua_player_type, "getSpellYName", pcl_getspellyname); //for remove spell
	lua_type_extendproto(&lua_player_type, "getSpellNameFromYName", pcl_getspellnamefromyname); //for remove spell
	lua_type_extendproto(&lua_player_type, "addEventXP", pcl_addEventXP); // adding experience to Event Ranking
	lua_type_extendproto(&lua_player_type, "mapSelection", pcl_mapselection);
	lua_type_extendproto(&lua_player_type, "deductArmor", pcl_deductarmor);
	lua_type_extendproto(&lua_player_type, "deductWeapon", pcl_deductweapon);
	lua_type_extendproto(&lua_player_type, "deductDura", pcl_deductdura);
	lua_type_extendproto(&lua_player_type, "deductDuraInv", pcl_deductdurainv);
	lua_type_extendproto(&lua_player_type, "deductDuraEquip", pcl_deductduraequip);
	lua_type_extendproto(&lua_player_type, "checkInvBod", pcl_checkinvbod);
	lua_type_extendproto(&lua_player_type, "durationAmount", pcl_durationamount);
	//lua_type_extendproto(&lua_player_type, "addGuide",pcl_addguide);
	//lua_type_extendproto(&lua_player_type, "delGuide",pcl_delguide);
	lua_type_extendproto(&lua_player_type, "powerBoard", pcl_powerboard);
	lua_type_extendproto(&lua_player_type, "lock", pcl_lock);
	lua_type_extendproto(&lua_player_type, "unlock", pcl_unlock);
	lua_type_extendproto(&lua_player_type, "swing", pcl_swing);
	lua_type_extendproto(&lua_player_type, "swingTarget", pcl_swingtarget);

	lua_type_extendproto(&lua_player_type, "getBankItem", pcl_getbankitem);
	lua_type_extendproto(&lua_player_type, "getBankItems", pcl_getbankitems);
	lua_type_extendproto(&lua_player_type, "bankDeposit", pcl_bankdeposit);
	lua_type_extendproto(&lua_player_type, "bankWithdraw", pcl_bankwithdraw);

	lua_type_extendproto(&lua_player_type, "getClanBankItems", pcl_getclanbankitems);
	lua_type_extendproto(&lua_player_type, "clanBankDeposit", pcl_clanbankdeposit);
	lua_type_extendproto(&lua_player_type, "clanBankWithdraw", pcl_clanbankwithdraw);

	lua_type_extendproto(&lua_player_type, "getSubpathBankItems", pcl_getsubpathbankitems);
	lua_type_extendproto(&lua_player_type, "subpathBankDeposit", pcl_subpathbankdeposit);
	lua_type_extendproto(&lua_player_type, "subpathBankWithdraw", pcl_subpathbankwithdraw);

	lua_type_extendproto(&lua_player_type, "addThreat", pcl_addthreat); //threat table
	lua_type_extendproto(&lua_player_type, "setThreat", pcl_setthreat); //threat table
	lua_type_extendproto(&lua_player_type, "addThreatGeneral", pcl_addthreatgeneral); //threat table

	lua_type_extendproto(&lua_player_type, "getCreationItems", pcl_getcreationitems);
	lua_type_extendproto(&lua_player_type, "getCreationAmounts", pcl_getcreationamounts);
	lua_type_extendproto(&lua_player_type, "getParcel", pcl_getparcel);
	lua_type_extendproto(&lua_player_type, "getParcelList", pcl_getparcellist);
	lua_type_extendproto(&lua_player_type, "removeParcel", pcl_removeparcel);
	lua_type_extendproto(&lua_player_type, "expireItem", pcl_expireitem);
	lua_type_extendproto(&lua_player_type, "testPacket", pcl_testpacket);
	lua_type_extendproto(&lua_player_type, "changeView", pcl_changeview);
	lua_type_extendproto(&lua_player_type, "setTimer", pcl_settimer);
	lua_type_extendproto(&lua_player_type, "addTime", pcl_addtime);
	lua_type_extendproto(&lua_player_type, "removeTime", pcl_removetime);
	lua_type_extendproto(&lua_player_type, "checkLevel", pcl_checklevel); //added for testing 11-26-16
...
int pcl_swingtarget(lua_State* state, USER* sd) {
	struct block_list* bl = ((struct block_list*)lua_type_topointer(state, lua_memberarg(1)));
	...
	if (bl->type == BL_MOB) {
		client_mob_damage(sd, lua_type_topointer(state, lua_memberarg(1)));
	}
	else if (bl->type == BL_PC) {
		client_pc_damage(sd, lua_type_topointer(state, lua_memberarg(1)));
	}
...
int pcl_menu(lua_State* state, USER* sd) {
	...
	int size = lua_objlen(state, lua_memberarg(2));
	...
	char* topic = lua_tostring(state, lua_memberarg(1));
...
int pcl_inputseq(lua_State* state, USER* sd) {
	...
	int previous = 0, next = 0;
	int size = lua_objlen(state, lua_memberarg(4));
	char* menuopts[size + 1];

	lua_pushnil(state);

	while (lua_next(state, lua_memberarg(4)) != 0) {
		// key is at -2, value at -1
		int index = lua_tonumber(state, -2);
		//if(index>0 && index<size) {
		menuopts[index] = lua_tostring(state, -1);
	...
	char* topic = lua_tostring(state, lua_memberarg(1));
	char* topic2 = lua_tostring(state, lua_memberarg(2));
	char* topic3 = lua_tostring(state, lua_memberarg(3));
...
int pcl_sendboardquestions(lua_State* state, USER* sd) {
	...
	unsigned int amount = lua_objlen(state, lua_memberarg(1));

	struct board_questionaire questions[amount + 1];
	memset(&questions, 0, sizeof(questions));

	lua_pushnil(state);
	while (lua_next(state, lua_memberarg(1)) != 0) {
		// key is at -2, value at -1
		int index = lua_tonumber(state, -2);
	...
	lua_pushnil(state);
	while (lua_next(state, lua_memberarg(3)) != 0) {
		// key is at -2, value at -1
		int index = lua_tonumber(state, -2);
	...
	int i = 0;
	while (i<args) {
		strcpy(&questions[a].header,lua_tostring(state, lua_memberarg(i+1)));
		strcpy(&questions[a].question,lua_tostring(state, lua_memberarg(i+2)));
		questions[a].inputLines = lua_tonumber(state, lua_memberarg(i+3));
		i+=3;
		a++;
	}
	lua_pop(state,args);

	client_sendBoardQuestionaire(sd, &questions, a);
...
int pcl_popup(lua_State* state, USER* sd) {
	...
	client_popup(sd, lua_tostring(state, lua_memberarg(1)));
	return 0;
}
...
int pcl_paperpopup(lua_State* state, USER* sd) {
	...
	int width = lua_tonumber(state, lua_memberarg(1));
	int height = lua_tonumber(state, lua_memberarg(2));
	unsigned char* string = lua_tostring(state, lua_memberarg(3));
...
/*int pcl_addguide(lua_State *state, USER *sd) {
	sl_checkargs(state, "s");
	int id=guidedb_id(lua_tostring(state,lua_memberarg(1)));
	int x;
	...
	for(x=0;x<256;x++) {
		...
		if(sd->status.guide[x]==0) {
			...
			client_sendguidespecific(sd,id);
			return 0;
		}
	}
	return 0;
...
int pcl_deductarmor(lua_State* state, USER* sd) {
	sl_checkargs(state, "n");
	client_deduct_armor(sd, lua_tonumber(state, lua_memberarg(1)));
	return 0;
}
int pcl_deductweapon(lua_State* state, USER* sd) {
	sl_checkargs(state, "n");
	client_deduct_weapon(sd, lua_tonumber(state, lua_memberarg(1)));
	return 0;
}
...
int pcl_flushduration(lua_State* state, USER* sd) {
	...
	for (x = 0; x < MAX_MAGIC_TIMERS; x++) {
		...
		if (flush == 1) {
			...
			if (bl != NULL) {
				lua_do_script_blargs(magicdb_yname(id), "uncast", 2, &sd->bl, bl);
			}
			else {
				lua_do_script_blargs(magicdb_yname(id), "uncast", 1, &sd->bl);
			}
		}
	}
...
/*int pcl_flushduration(lua_State *state, USER *sd) {
	...
	for(x = 0; x < MAX_MAGIC_TIMERS; x++) {
		...
		if(id > 0) {
			sd->status.dura_aether[x].duration = 0;
			client_send_duration(sd,sd->status.dura_aether[x].id,0);
			sd->status.dura_aether[x].caster_id = 0;
			map_foreachinarea(client_send_animation, sd->bl.m, sd->bl.x, sd->bl.y, AREA, BL_PC, sd->status.dura_aether[x].animation, &sd->bl, -1);
			sd->status.dura_aether[x].animation = 0;
...
int pcl_killcount(lua_State* state, USER* sd) {
	...
	if (lua_isnumber(state, lua_memberarg(1))) {
		...
		sl_checkargs(state, "s");
		id = mobdb_id(lua_tostring(state, lua_memberarg(1)));
	}
...
int pcl_flushkills(lua_State* state, USER* sd) {
	...
	if (lua_isnil(state, lua_memberarg(1)) == 0) {
		if (lua_isnumber(state, lua_memberarg(1))) {
			//sl_checkargs(state, "n");
			flushid = lua_tonumber(state, lua_memberarg(1));
		}
		else {
			//sl_checkargs(state, "s");
			flushid = mobdb_id(lua_tostring(state, lua_memberarg(1)));
		}
	}
...
int pcl_showhealth(lua_State* state, USER* sd) {
	sl_checkargs(state, "nn");
	int type = lua_tonumber(state, lua_memberarg(1));
	int damage = lua_tonumber(state, lua_memberarg(2));
	//printf("Damage: %d\n",damage);
...
int pcl_addclan(lua_State* state, USER* sd) {
	...
	sl_checkargs(state, "s");
	char* c_name = lua_tostring(state, lua_memberarg(1));
...
int pcl_hasequipped(lua_State* state, USER* sd) {
	...
	if (lua_isstring(state, lua_memberarg(1))) {
		id = itemdb_id(lua_tostring(state, lua_memberarg(1)));
	}
	else {
		id = lua_tonumber(state, lua_memberarg(1));
...
int pcl_sendurl(lua_State* state, USER* sd) {
	int type = lua_tonumber(state, lua_memberarg(1));
	char* url = lua_tostring(state, lua_memberarg(2));
...
int pcl_dialog(lua_State* state, USER* sd) {
	...
	char* msg = lua_tostring(state, lua_memberarg(1));
...
int pcl_setCaptchaKey(lua_State* state, USER* sd) {
	...
	char* key = lua_tostring(state, lua_memberarg(1));
...
int pcl_setAccountBan(lua_State* state, USER* sd) {
	int banned = lua_tonumber(state, lua_memberarg(1));
...
{
	sl_checkargs(state, "n");
	sd->status.heroes = lua_tonumber(state, lua_memberarg(1));
...
int pcl_updatePath(lua_State* state, USER* sd) {
	int path = lua_tonumber(state, lua_memberarg(1));
	int mark = lua_tonumber(state, lua_memberarg(2));
...
int pcl_updateCountry(lua_State* state, USER* sd) {
	unsigned int country = lua_tonumber(state, lua_memberarg(1));
...
int pcl_updateMail(lua_State* state, USER* sd) {
	char* name = lua_tostring(state, lua_memberarg(1));
...
{
	sl_checkargs(state, "n");
	int kan = lua_tonumber(state, lua_memberarg(1));
...
int pcl_useitem(lua_State* state, USER* sd) {
	sl_checkargs(state, "s");
	char* itemname = lua_tostring(state, lua_memberarg(1));
	int i;
...
int pcl_setMiniMapToggle(lua_State* state, USER* sd) {
	sl_checkargs(state, "n");
	unsigned int miniMapToggle = lua_tonumber(state, lua_memberarg(1));
...
int pcl_addhealth(lua_State* state, USER* sd) {
	...
	if (bl != NULL) {
		if (damage > 0) lua_do_script_blargs("player_combat", "on_healed", 2, &sd->bl, bl);
	}
	else {
		if (damage > 0) lua_do_script_blargs("player_combat", "on_healed", 1, &sd->bl);
	}
...
int pcl_sendminitext(lua_State* state, USER* sd) {
	...
	msg = lua_tostring(state, lua_memberarg(1));
...
int pcl_setduration(lua_State* state, USER* sd) {
	...
	time = lua_tonumber(state, lua_memberarg(2));
	if (time < 1000 && time > 0) time = 1000;
	caster_id = lua_tonumber(state, lua_memberarg(3)); //sets duration from specific caster
	recast = lua_tonumber(state, lua_memberarg(4));
	...
	for (x = 0; x < MAX_MAGIC_TIMERS; x++) { //MAX_DURATIMER
		...
		if (mid == id && time <= 0 && sd->status.dura_aether[x].caster_id == caster_id && alreadycast == 1) {
			...
			if (bl != NULL) {
				lua_do_script_blargs(magicdb_yname(mid), "uncast", 2, &sd->bl, bl);
			}
			else {
				lua_do_script_blargs(magicdb_yname(mid), "uncast", 1, &sd->bl);
			}
			...
			//	client_send_duration(sd, id, time / 1000);
			//} else if (caster_id == 0) {
			sd->status.dura_aether[x].id = id;
			sd->status.dura_aether[x].duration = time;
			sd->status.dura_aether[x].caster_id = caster_id;
...
int pcl_hasdurationid(lua_State* state, USER* sd) {
	...
	id = magicdb_id(lua_tostring(state, lua_memberarg(1)));
	caster_id = lua_tonumber(state, lua_memberarg(2)); //checks for duration from specific caster
...
int pcl_addlegend(lua_State* state, USER* sd) {
	...
	icon = lua_tonumber(state, lua_memberarg(3));
	color = lua_tonumber(state, lua_memberarg(4));
	tchaid = lua_tonumber(state, lua_memberarg(5));

	for (x = 0; x < MAX_LEGENDS; x++) {
		if (strcmpi(sd->status.legends[x].name, "") == 0 && strcmpi(sd->status.legends[x + 1].name, "") == 0) {
			strcpy(sd->status.legends[x].text, lua_tostring(state, lua_memberarg(1)));
			strcpy(sd->status.legends[x].name, lua_tostring(state, lua_memberarg(2)));
...
int pcl_haslegend(lua_State* state, USER* sd) {
	sl_checkargs(state, "s");
	int i;
	char* legend = lua_tostring(state, lua_memberarg(1));
...
int pcl_buy(lua_State* state, USER* sd) {
	sl_checkargs(state, "stt");
	int x;
	unsigned int amount = lua_objlen(state, lua_memberarg(2));
	...
	lua_pushnil(state);
	while (lua_next(state, lua_memberarg(5)) != 0) {
		int index = lua_tonumber(state, -2);
	...
	lua_pushnil(state);
	while (lua_next(state, lua_memberarg(6)) != 0) {
		int index = lua_tonumber(state, -2);
	...
	lua_pushnil(state);
	while (lua_next(state, lua_memberarg(7)) != 0) {
		int index = lua_tonumber(state, -2);
	...
	lua_pushnil(state);
	while (lua_next(state, lua_memberarg(8)) != 0) {
		int index = lua_tonumber(state, -2);
	...
	dialog = lua_tostring(state, lua_memberarg(1));
...
int pcl_mapselection(lua_State* state, USER* sd) {
	...
	wm = lua_tostring(state, lua_memberarg(1));

	int index = lua_objlen(state, lua_memberarg(2));
...
int pcl_input(lua_State* state, USER* sd) {
	...
	char* dialog = lua_tostring(state, lua_memberarg(1));
	char* name = NULL;

	int temp = 0;

	if (lua_isnumber(state, lua_memberarg(2))) name = itemdb_name(lua_tonumber(state, lua_memberarg(2)));
	else if (lua_isstring(state, lua_memberarg(2))) name = lua_tostring(state, lua_memberarg(2));
	...
	//if (strcmp(name,"") != 0) client_input(sd,0,dialog,name);
	//else client_input(sd,sd->last_click,dialog,"");
...
int pcl_removelegendbyname(lua_State* state, USER* sd) {
	sl_checkargs(state, "s");
	char* find = lua_tostring(state, lua_memberarg(1));
	int x;
...
int pcl_removelegendbycolor(lua_State* state, USER* sd) {
	sl_checkargs(state, "n");
	int find = lua_tonumber(state, lua_memberarg(1));
	int x, count = 0;
...
int pcl_removeinventoryitem(lua_State* state, USER* sd) {
	...
	amount = lua_tonumber(state, lua_memberarg(2));
	type = lua_tonumber(state, lua_memberarg(3));
	owner = lua_tonumber(state, lua_memberarg(4));
	engrave = lua_tostring(state, lua_memberarg(5));
	max = amount;
...
int pcl_removeitemslot(lua_State* state, USER* sd) {
	...
	slot = lua_tonumber(state, lua_memberarg(1));
	amount = lua_tonumber(state, lua_memberarg(2));
	type = lua_tonumber(state, lua_memberarg(3));
	max = amount;
...
int pcl_addGift(lua_State* state, USER* sd) {
	/*int x;
	unsigned int size=lua_objlen(state,lua_memberarg(1));
...
int pcl_retrieveGift(lua_State* state, USER* sd) {
	/*unsigned int giftid = lua_tonumber(state, lua_memberarg(1));
...
int pcl_additem(lua_State* state, USER* sd) {
	...
	dura = lua_tonumber(state, lua_memberarg(3));
	owner = lua_tonumber(state, lua_memberarg(4));
	time = lua_tonumber(state, lua_memberarg(5));

	engrave = lua_tostring(state, lua_memberarg(6));

	customLook = lua_tonumber(state, lua_memberarg(7));
	customLookColor = lua_tonumber(state, lua_memberarg(8));
	customIcon = lua_tonumber(state, lua_memberarg(9));
	customIconColor = lua_tonumber(state, lua_memberarg(10));
	protected = lua_tonumber(state, lua_memberarg(11));
	note = lua_tostring(state, lua_memberarg(12));
	custom = lua_tonumber(state, lua_memberarg(13));
...
int pcl_showboard(lua_State* state, USER* sd) {
	...
	if (lua_isnumber(state, lua_memberarg(1))) {
		...
		sl_checkargs(state, "s");
		id = boarddb_id(lua_tostring(state, lua_memberarg(1)));
	}
...
int pcl_showpost(lua_State* state, USER* sd) {
	...
	post = lua_tonumber(state, lua_memberarg(2));
...
int pcl_hasitem(lua_State* state, USER* sd) {
	...
	amount = abs(lua_tonumber(state, lua_memberarg(2)));
	dura = lua_tonumber(state, lua_memberarg(3));
	owner = lua_tonumber(state, lua_memberarg(4));
	engrave = lua_tostring(state, lua_memberarg(5));
	customLook = lua_tonumber(state, lua_memberarg(6));
	customLookColor = lua_tonumber(state, lua_memberarg(7));
	customIcon = lua_tonumber(state, lua_memberarg(8));
	customIconColor = lua_tonumber(state, lua_memberarg(9));
	note = lua_tostring(state, lua_memberarg(10));
...
int pcl_addspell(lua_State* state, USER* sd) {
	...
	if (lua_isnumber(state, lua_memberarg(1))) {
		spell = lua_tonumber(state, lua_memberarg(1));
	}
	else if (lua_isstring(state, lua_memberarg(1))) {
		spell = magicdb_id(lua_tostring(state, lua_memberarg(1)));
	}
	else {
		lua_pushnil(state);
		return 0;
	...
	for (x = 0; x < 52; x++) {
		if (sd->status.skill[x] <= 0) { //vacant spot
			lua_do_script_blargs(magicdb_yname(spell), "on_learn", 1, &sd->bl);
...
int pcl_hasspell(lua_State* state, USER* sd) {
	sl_checkargs(state, "s");
	int spell = magicdb_id(lua_tostring(state, lua_memberarg(1)));
	//char name[32];
	int x;
...
int pcl_addEventXP(lua_State* state, USER* sd) {
	...
	int eventid = -1;
	char eventname[40] = "";
	unsigned int score = lua_tonumber(state, lua_memberarg(2));
	unsigned char sql_query[500] = "";

	if (lua_isnumber(state, lua_memberarg(1))) {
		sl_checkargs(state, "nn");
		eventid = lua_tonumber(state, lua_memberarg(1));
	}
	else {
		sl_checkargs(state, "sn");
		strcpy(eventname, lua_tostring(state, lua_memberarg(1)));
	}
...
int pcl_hasspace(lua_State* state, USER* sd) {
	...
	int incomingAmount = lua_tonumber(state, lua_memberarg(2));
	int owner = lua_tonumber(state, lua_memberarg(3));
	int stackAmount = itemdb_stackamount(id);
	bool hasOpenInventorySlot = false;

	engrave = lua_tostring(state, lua_memberarg(4));
	customLook = lua_tonumber(state, lua_memberarg(5));
	customLookColor = lua_tonumber(state, lua_memberarg(6));
	customIcon = lua_tonumber(state, lua_memberarg(7));
	customIconColor = lua_tonumber(state, lua_memberarg(8));
	protected = lua_tonumber(state, lua_memberarg(9));
	note = lua_tostring(state, lua_memberarg(10));
...
int pcl_removespell(lua_State* state, USER* sd) {
	...
	for (x = 0; x < MAX_SPELLS; x++) {
		if (sd->status.skill[x] == spell) {
			lua_do_script_blargs(magicdb_yname(spell), "on_forget", 1, &sd->bl);
...
int pcl_bankdeposit(lua_State* state, USER* sd) {
	...
	deposit = -1;
	item = lua_tonumber(state, lua_memberarg(1));
	amount = lua_tonumber(state, lua_memberarg(2));
	owner = lua_tonumber(state, lua_memberarg(3));
	time = lua_tonumber(state, lua_memberarg(4));
	engrave = lua_tostring(state, lua_memberarg(5));
	protected = lua_tonumber(state, lua_memberarg(6));
	customIcon = lua_tonumber(state, lua_memberarg(7));
	customIconColor = lua_tonumber(state, lua_memberarg(8));
	customLook = lua_tonumber(state, lua_memberarg(9));
	customLookColor = lua_tonumber(state, lua_memberarg(10));
...
int pcl_givexp(lua_State* state, USER* sd) {
	sl_checkargs(state, "n");
	pc_givexp(sd, lua_tonumber(state, lua_memberarg(1)), xp_rate);
}
...
int pcl_addthreatgeneral(lua_State* state, USER* sd) {
	...
	amount = lua_tonumber(state, lua_memberarg(1));
...
int pcl_talkself(lua_State* state, USER* sd) {
	...
	int type = lua_tonumber(state, lua_memberarg(1));
	char* msg = lua_tostring(state, lua_memberarg(2));
	unsigned int id = lua_tonumber(state, lua_memberarg(3));
	int msglen = strlen(msg);
...
int pcl_sendhealth(lua_State* state, USER* sd) {
	...
	client_send_pc_healthscript(sd, damage, critical);
}

int pcl_sendmail(lua_State* state, USER* sd) {
	sl_checkargs(state, "sss");
	char* to_user = lua_tostring(state, lua_memberarg(1));
	char* topic = lua_tostring(state, lua_memberarg(2));
	char* message = lua_tostring(state, lua_memberarg(3));
...
int pcl_pickup(lua_State* state, USER* sd) {
	sl_checkargs(state, "n");
	unsigned int id = lua_tonumber(state, lua_memberarg(1));
...
int pcl_forceequip(lua_State* state, USER* sd) {
	...
	int slot = lua_tonumber(state, lua_memberarg(2));
...
int pcl_stripequip(lua_State* state, USER* sd) {
	...
	sl_checkargs(state, "n");
	unsigned int equipType = lua_tonumber(state, lua_memberarg(1));
	FLOORITEM* fl;

	destroy = lua_tonumber(state, lua_memberarg(2));			//if 1 then will be destroyed
	force = lua_tonumber(state, lua_memberarg(3));			//if 1 then will be force to drop if inv full
...
int pcl_move(lua_State* state, USER* sd) {
	//sl_checkargs(state, "n");
	char speed = lua_tonumber(state, lua_memberarg(1));
...
int pcl_deductdura(lua_State* state, USER* sd) {
	sl_checkargs(state, "nn");
	int equip = lua_tonumber(state, lua_memberarg(1));
	unsigned int amount = lua_tonumber(state, lua_memberarg(2));
...
int pcl_deductdurainv(lua_State* state, USER* sd) {
	sl_checkargs(state, "nn");
	int slot = lua_tonumber(state, lua_memberarg(1));
	unsigned int amount = lua_tonumber(state, lua_memberarg(2));
...
int pcl_getcreationitems(lua_State* state, USER* sd) {
	sl_checkargs(state, "n");
	int len = lua_tonumber(state, lua_memberarg(1));
	int curitem;
...
int pcl_getcreationamounts(lua_State* state, USER* sd) {
	sl_checkargs(state, "nn");
	int len = lua_tonumber(state, lua_memberarg(1));
	int item = lua_tonumber(state, lua_memberarg(2));
...
int pcl_removeparcel(lua_State* state, USER* sd) {
	sl_checkargs(state, "nnnn");
	int sender = lua_tonumber(state, lua_memberarg(1));
	unsigned int item = lua_tonumber(state, lua_memberarg(2));
	unsigned int amount = lua_tonumber(state, lua_memberarg(3));
	int pos = lua_tonumber(state, lua_memberarg(4));
	int owner = lua_tonumber(state, lua_memberarg(5));
	char* engrave = lua_tostring(state, lua_memberarg(6));
	char npcflag = lua_tonumber(state, lua_memberarg(7));
	unsigned int customLook = lua_tonumber(state, lua_memberarg(8));
	unsigned int customLookColor = lua_tonumber(state, lua_memberarg(9));
	unsigned int customIcon = lua_tonumber(state, lua_memberarg(10));
	unsigned int customIconColor = lua_tonumber(state, lua_memberarg(11));
...
int pcl_logbuysell(lua_State* state, USER* sd) {
	sl_checkargs(state, "nnnn");
	unsigned int item = lua_tonumber(state, lua_memberarg(1));
	unsigned int amount = lua_tonumber(state, lua_memberarg(2));
	unsigned int money = lua_tonumber(state, lua_memberarg(3));
	char buysell = lua_tonumber(state, lua_memberarg(4));
...
int pcl_settimevalues(lua_State* state, USER* sd) {
	sl_checkargs(state, "n");
	int i;
	unsigned int newval = lua_tonumber(state, lua_memberarg(1));
...
int pcl_getunknownspells(lua_State* state, USER* sd) {
	...
	unsigned int class1 = lua_tonumber(state, lua_memberarg(1));
	unsigned int class2 = lua_tonumber(state, lua_memberarg(2));
...
int pcl_getallclassspells(lua_State* state, USER* sd) { // Used to display all spells in game
	...
	unsigned int class = lua_tonumber(state, lua_memberarg(1));
...
int pcl_settimer(lua_State* state, USER* sd) {
	sl_checkargs(state, "nn");
	char type = lua_tonumber(state, lua_memberarg(1));
	unsigned int length = lua_tonumber(state, lua_memberarg(2));
...
int pcl_addtime(lua_State* state, USER* sd) {
	sl_checkargs(state, "n");
	unsigned int addition = lua_tonumber(state, lua_memberarg(1));
...
void maplua_registry_staticinit() {
	mapregl_type = lua_type_new("Mapregistry", 0);
...
void gamelua_registry_staticinit() {
	gameregl_type = lua_type_new("Gameregistry", 0);
...
void acctlua_registry_staticinit() {
	acctregl_type = lua_type_new("Accountregistry", 0);
