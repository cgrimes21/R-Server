...
/*int client_encrypt(int fd)
{
	...
	{	//rtk_crypt2(WFIFOP(fd,0),sd->status.EncKey);
		rtk_crypt2(WFIFOP(fd,0),&(sd->status.EncKey));
...
int client_decrypt(int fd)
{
	...
	{
		rtk_crypt2(RFIFOP(fd,0),&(sd->status.EncKey));
	} else {
...
int client_broadcasttogm_sub(struct block_list* bl, va_list ap) {
	USER* sd = NULL;
	char* msg = NULL;
	//char buf[256];
	int len = 0;
...
int client_broadcasttogm(char* msg, int m) {
	if (m == -1) {
		for (int x = 0; x < 65535; x++) {
			if (map_isloaded(x)) {
				map_foreachinarea(client_broadcasttogm_sub, x, 1, 1, SAMEMAP, BL_PC, msg);
			}
		}
	}
	else {
		map_foreachinarea(client_broadcasttogm_sub, m, 1, 1, SAMEMAP, BL_PC, msg);
	}
	return 0;
}
...
int client_npc_move(struct block_list* bl, va_list ap) {
	...
	client_send(buf, 32, &nd->bl, AREA_WOS); //come back
	FREE(buf);
	...
	for (x = 0; x < MAX_LEGENDS; x++) {
		if (strlen(sd->status.legends[x].text) && strlen(sd->status.legends[x].name)) {
			...
			if (sd->status.legends[x].tchaid > 0) {
				char* name = client_get_name(sd->status.legends[x].tchaid);
...
int client_noparse_walk(USER* sd, char speed) {
	...
	if (sd->optFlags & optFlag_stealth) {
		...
		client_send(buf, 32, &sd->bl, AREA_WOS);
	}
	FREE(buf);
...
int client_check_dura(USER* sd, int equip) {
	...
	if (percentage <= .25 && sd->status.equip[equip].repair == 1) {
		...
		client_send_msg(sd, 5, buf);
		//client_send_minitext(sd, buf,strlen(buf));
...
int client_check_inv_bod(USER* sd) { // handles items in inventory
	float percentage;
	int id;
	char buf[255];
	char escape[255];
...
int client_send_novice_message(USER* sd, unsigned char* msg, int msglen) {
	...
	if (sd->status.tutor == 0) { // Added to simulate to a non-tutor that their message was indeed sent (since they won't be able to see it)
		client_send_msg(sd, 11, buf2); // This function will make it appear on their screen
	}
...
int client_send_mob_say(struct block_list* bl, va_list ap) {
	...
	if (client_distance(&sd->bl,&mob->bl) <= 10) {
		sd->last_click=bl->id;
		sl_async_freeco(sd);
...
int client_send_mob_yell(struct block_list* bl, va_list ap) {
	...
	if (client_distance(&sd->bl,&mob->bl) <= 20) {
		sd->last_click=bl->id;
		sl_async_freeco(sd);
...
int client_script_menu(USER* sd, int id, char* dialog, char* menu[], int size) { // this function does not appear to get called any where at all
	...
	/*if(itemdb_thrown(sd->status.equip[EQ_WEAP].id)) {// && sd->time < 3) {
		if(sd->rangeTarget > 0) {
			...
			if(sd->bl.id == tbl->id || (sd->bl.x == tbl->x && sd->bl.y == tbl->y)) {
				sd->rangeTarget = 0;
				client_send_minitext(sd,"You cannot shoot at yourself!");
				return 0;
			}
			if(tbl->m != sd->bl.m || (tbl->x > sd->bl.x + 8 || tbl->x < sd->bl.x - 8) || (tbl->y > sd->bl.y + 8 || tbl->y < sd->bl.y - 8)) {
				sd->rangeTarget = 0;
				client_send_minitext(sd,"Your target has escaped!");
				return 0;
			}
			if(tbl) {
				long health = 0;
	...
	if (map[sd->bl.m].show_ghosts) {
		if (sd->status.state == 1 && (src_sd->bl.id != sd->bl.id)) {
			if (src_sd->status.state != 1 && !(src_sd->optFlags & optFlag_ghosts)) {
				...
				client_char_specific(src_sd->bl.id, sd->bl.id);
			}
...
int client_click_on_player(USER* sd, struct block_list* bl) {
	...
	for (x = 0; x < 14; x++) {
		...
		if (tsd->status.equip[x].id > 0 && (itemdb_type(tsd->status.equip[x].id) >= 3) && (itemdb_type(tsd->status.equip[x].id) <= 16)) {
			...
			sprintf(buff, map_msg[client_map_msgnum(tsd, x)].message, nameof);
	...
	*/
	...
	for (x = 0; x < MAX_LEGENDS; x++) {
		if (strlen(tsd->status.legends[x].text) && strlen(tsd->status.legends[x].name)) {
			...
			if (tsd->status.legends[x].tchaid > 0) {
				char* name = client_get_name(tsd->status.legends[x].tchaid);
	...
	if (client_is_registered(tsd->status.id) > 0) WFIFOB(sd->fd, len + 7) = 1;
...
int client_grouphealth_update(USER* sd) {
	...
	nextlevelxp=client_get_lvl_xp(sd->status.level+1);

			//maxtnl-=client_get_lvl_xp(99)-1;
...
int client_change_status(USER* sd, int type) {
	...
	case 0x00: //Ride/something else
		if (RFIFOB(sd->fd, 7) == 1) {
			if (sd->status.state == 0) {
				client_find_mount(sd);

				if (sd->status.state == 0) client_send_minitext(sd, "Good try, but there is nothing here that you can ride.");
			}
			else if (sd->status.state == 1) {
				client_send_minitext(sd, "Spirits can't do that.");
			}
			else if (sd->status.state == 2) {
				client_send_minitext(sd, "Good try, but there is nothing here that you can ride.");
			}
	...
	case 0x01: //Whisper (F5)
		...
		if (sd->status.settingFlags & FLAG_WHISPER) {
			client_send_minitext(sd, "Listen to whisper:ON");
		}
		else {
			client_send_minitext(sd, "Listen to whisper:OFF");
		}
		client_send_status(sd, NULL);
		break;
	case 0x02: //group
	...
	case 0x03: //Shout
		...
		if (sd->status.settingFlags & FLAG_SHOUT) {
			client_send_minitext(sd, "Listen to shout  :ON");
		}
		else {
			client_send_minitext(sd, "Listen to shout  :OFF");
		}
		client_send_status(sd, NULL);
		break;
	case 0x04: //Advice
		...
		if (sd->status.settingFlags & FLAG_ADVICE) {
			client_send_minitext(sd, "Listen to advice :ON");
		}
		else {
			client_send_minitext(sd, "Listen to advice :OFF");
		}
		client_send_status(sd, NULL);
		break;
	case 0x05: //Magic
		...
		if (sd->status.settingFlags & FLAG_MAGIC) {
			client_send_minitext(sd, "Believe in magic :ON");
		}
		else {
			client_send_minitext(sd, "Believe in magic :OFF");
		}
		client_send_status(sd, NULL);
		break;
	case 0x06: //Weather
	...
	case 0x07: //Realm center (F4)
		...
		if (sd->status.settingFlags & FLAG_REALM) {
			client_send_minitext(sd, "Realm-centered   :ON");
		}
		else {
			client_send_minitext(sd, "Realm-centered   :OFF");
		}
		client_send_status(sd, NULL);
		break;
	case 0x08: //exchange
	...
	case 0x09: //Fast move
		...
		if (sd->status.settingFlags & FLAG_FASTMOVE) {
			client_send_minitext(sd, "Fast Move        :ON");
		}
		else {
			client_send_minitext(sd, "Fast Move        :OFF");
		}
		client_send_status(sd, NULL);
		break;
	case 10: //Clan chat
		...
		if (sd->status.clan_chat) {
			client_send_minitext(sd, "Clan whisper     :ON");
		}
		else {
			client_send_minitext(sd, "Clan whisper     :OFF");
		}
		break;
	case 13: //Sound
	...
	case 14: //Helm
		...
		if (sd->status.settingFlags & FLAG_HELM) {
			client_send_minitext(sd, "Show Helmet      :ON");
			pc_setglobalreg(sd, "show_helmet", 1);			//Added 4/6/17 to give registry for helmet status
		}
		else {
			client_send_minitext(sd, "Show Helmet      :OFF");
			pc_setglobalreg(sd, "show_helmet", 0);          //Added 4/6/17 to give registry for helmet status
		}
		client_send_status(sd, NULL);
		client_send_char_area(sd);
		client_get_char_area(sd);
		//map_foreachinarea(client_update_state,sd->bl.m,sd->bl.x,sd->bl.y,AREA,BL_PC,sd); // was commented
		break;
...
int client_parse_exchange(USER* sd) {
	...
	case 0: //this is a "initiation", so we do this
		...
		if (sd->status.gm_level || !(tsd->optFlags & optFlag_stealth))
			client_start_exchange(sd, SWAP32(RFIFOL(sd->fd, 6)));
		break;
	case 1: //amount?
		...
		if (sd->status.inventory[id].amount > 1) {
			...
			tsd = map_id2sd(SWAP32(RFIFOL(sd->fd, 6)));
			client_exchange_add_item(sd, tsd, id, 1);
		}
		else //client_hacker(sd->status.name, "Attempted to exchange-hack-add a blank slot");
			break;
	case 2: //after ask amount
		...
		if (amount > 0 && (sd->status.inventory[id].id && amount <= sd->status.inventory[id].amount)) {
			client_exchange_add_item(sd, tsd, id, amount);
		}
		else if (!sd->status.inventory[id].id || amount == 0) {
			//client_hacker(sd->status.name, "Attempted to exchange-hack-add a blank slot");
		}
		break;
	...
	case 4: //Quit exchange
		tsd = map_id2sd(SWAP32(RFIFOL(sd->fd, 6)));
		client_exchange_message(sd, "Exchange cancelled.", 4, 0);
		if (tsd) client_exchange_message(tsd, "Exchange cancelled.", 4, 0);
		client_exchange_close(sd);
		client_exchange_close(tsd);
		break;
	case 5: //Finish exchange
		...
		if (sd->exchange.gold > sd->status.money) {
			client_exchange_message(sd, "You do not have that amount.", 4, 0);
			client_exchange_message(tsd, "Exchange cancelled.", 4, 0);
			client_exchange_close(sd);
			client_exchange_close(tsd);
		}
		else
			if (tsd) { client_exchange_send_ok(sd, tsd); }
			else { client_exchange_close(sd); }
...
int client_can_move(USER* sd, int direct) {
	...
	map_foreachincell(client_can_move_sub, sd->bl.m, sd->bl.x, sd->bl.y, BL_MOB, sd);
	map_foreachincell(client_can_move_sub, sd->bl.m, sd->bl.x, sd->bl.y, BL_PC, sd);
	map_foreachincell(client_can_move_sub, sd->bl.m, nx, ny, BL_PC, sd);
	if (client_object_can_move(sd->bl.m, nx, ny, direct)) {
		sd->canmove = 1;
	}
	return sd->canmove;
}
