...
int login_client_accept(int fd) {
...
int login_client_message(int fd, char code, char* buff) {
...
int login_client_sendurl(int fd, int type, char* url)
{
	int ulen = strlen(url);
	int len = 0;
...
int login_client_debug(unsigned char* stringthing, int len) {
	int i;
...
int login_client_parse(int fd) {
	...
	case 0x02:
		...
		if (char_fd) {
			...
			login_client_message(fd, 0x03, login_msg[LGN_ERRDB]);
		}
		break;
	case 0x03:
		...
		if (maintenance_mode()) {
			if (maintenance_override(RFIFOP(fd, 6), RFIFOB(fd, 5)) == 0) {
				login_client_message(fd, 0x03, "Server is undergoing maintenance. Please visit www.RetroTK.com or the RetroTK facebook group for more details.");
				break;
			}
		}
	...
	case 0x10:
		...
		login_client_debug(RFIFOP(fd, 5), SWAP16(RFIFOW(fd, 1)) - 5);
		memset(name, 0, 31);
